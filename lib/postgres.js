...
PG.prototype.all = function all(model, filter, callback) {
    /* 
     * PATCH: Не удалять, а ставить флаг "delete", если в схеме указан пареметр notDelete
     *  Выборка только тех элементов, флаг "delete" у которых стоит в false
     */
    if( this._models[model].settings.notDelete )
    {
        if( filter === null ){
