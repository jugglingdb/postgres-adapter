...
PG.prototype.define = function (descr) {
    ...
    Object.keys(descr.properties).forEach(function(key) {
        var property = descr.properties[property];
        if (property.index || property.unique) {
            ...
            var idx = {columns: key};
            if (property.unique) idx.kind = 'UNIQUE';
...
function getIndexStatus(model, cb) {
  ...
  this.query('SELECT b.relname AS "Name",a.indisunique AS "Unique",ARRAY_TO_STRING(ARRAY_AGG(c.attname), ',') AS "Columns"' +
             '  FROM' +
             '    (SELECT indrelid,indexrelid,UNNEST(indkey) AS columnpos,indisunique' +
             '      FROM pg_index' +
             '      WHERE indrelid=\'' + this.table(model) + '\'::regclass) a' +
             '    INNER JOIN pg_class b' +
             '      ON a.indexrelid = b.oid' +
             '    INNER JOIN pg_attribute c' +
             '      ON a.indrelid = c.attrelid' +
             '        AND a.columnpos = c.attnum' +
    ...
    if (indexNames) {
        indexNames.forEach(function (indexName) {
            ...
            if (found && indexHasNotBeenDeleted.call(self, model, indexName)) {
                ...
                if (found.Columns.length !== columns.length || columnOrderChanged(columns, found.Columns)) {
                    sql = sql.concat(modifyIndexInActual.call(self, model, indexName));
                }
            }
        });
    }
...
function modifyIndexInActual(model, indexName) {
    var self = this;
    var sql = [];
    sql.push(dropIndexFromActual.call(self, model, indexName));
    sql.push(addIndexToActual.call(self, model, indexName));
    return sql;
};
...
function getIndexesToDrop(model, actualIndexes) {
    ...
    actualIndexes.forEach(function (actualIndex) {
        if (actualIndexNotPresentInModel(actualIndex.Name, model)) {
            sql.push(dropIndexFromActual.call(self, model, actualIndex.Name));
        }
    });
    return sql;
...
function dropIndexFromActual(model, indexName) {
    var self = this;
    var i = self._models[model].settings.indexes[indexName];
    if (i.kind && i.kind.toLowerCase() === 'unique')
...
function applySqlIndexChanges(model, pendingChanges, done, idx) {
    ...
    if (idx < pendingChanges.length) {
        self.query(pendingChanges[idx] + ';', function(err) {
            if (err) console.log(err);
            applySqlIndexChanges.call(self, pendingChanges, done, idx + 1);
        });
    } else {
        done();
    }
